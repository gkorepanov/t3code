...
function assignMessageTimestamps(
  ...
  const step =
    messages.length > 1 ? Math.max(1, Math.floor((fallbackEnd - fallbackStart) / (messages.length - 1))) : 0;
...
export function resolveImportedThreadTitle(input: {
  ...
  const candidates = [
    input.title,
    input.indexTitle,
    input.firstUserMessage ? normalizeImportedUserText(input.firstUserMessage) : input.firstUserMessage,
  ];
