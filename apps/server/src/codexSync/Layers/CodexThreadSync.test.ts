...
describe("CodexThreadSyncLive", () => {
  it("imports missing Codex threads, skips archived/existing ones, and persists transcript state", async () => {
    ...
    try {
      ...
      const projectionSnapshotQuery = await runtime.runPromise(Effect.service(ProjectionSnapshotQuery));
      ...
      expect(importedThread?.messages.map((message) => ({
        role: message.role,
        text: message.text,
      }))).toEqual([
      ...
      expect(rerunSnapshot.threads.filter((thread) => thread.projectId === importedProject?.id)).toHaveLength(1);
