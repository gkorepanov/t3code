  ...
  Effect.gen(function* () {
    ...
    const syncThreads: CodexThreadSyncShape["syncThreads"] = (rawInput) =>
      Effect.gen(function* () {
        ...
        const configuredCodexSettings = serverSettings.providers.codex;
        const codexBinaryPath =
          normalizeOptionalPath(input.codexBinaryPath) ??
          normalizeOptionalPath(configuredCodexSettings.binaryPath);
        const codexHomeOverride =
          normalizeOptionalPath(input.codexHomePath) ??
          normalizeOptionalPath(configuredCodexSettings.homePath);
        const codexHomePath = codexHomeOverride ?? path.join(os.homedir(), ".codex");
            ...
            Effect.gen(function* () {
              ...
              if (!projectId) {
                ...
                yield* orchestrationEngine.dispatch({
                  ...
                  defaultModelSelection: defaultCodexModelSelection(),
              ...
              yield* orchestrationEngine.dispatch({
                ...
                modelSelection: project?.defaultModelSelection ?? defaultCodexModelSelection(),
                runtimeMode: "full-access",
