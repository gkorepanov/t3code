...
import { readResumeCursorThreadId, readSyncedCodexThreadId } from "../codexSync.ts";
import {
  CodexThreadArchive,
  type CodexThreadArchiveShape,
} from "../Services/CodexThreadArchive.ts";
    ...
    Effect.gen(function* () {
      ...
      const archiveThread: CodexThreadArchiveShape["archiveThread"] = (rawInput) =>
        Effect.gen(function* () {
          ...
          const codexThreadId =
            readResumeCursorThreadId(runtimeRow?.resumeCursor) ??
            readSyncedCodexThreadId(input.threadId);
          ...
          const runtimeMode: RuntimeMode =
            thread?.runtimeMode ?? runtimeRow?.runtimeMode ?? "full-access";
