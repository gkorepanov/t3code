...
export function buildThreadTitlePrompt(input: ThreadTitlePromptInput) {
  const prompt = buildPromptFromMessage({
    ...
    rules: [
      "Every title must start with exactly one emoji.",
      "Emoji must be semantic and add a useful clue, such as domain, UI surface, data shape, failure mode, file type, or workflow.",
      ...
      "If images are attached, use them as primary context for visual/UI issues.",
      "Good examples: 🏞️ FIX remote images in T3; 📎 Remote links click through; 🔎 upbit.pyx GKV3/MMSimple hunt; 📈 Passive maker CFG; 🧵 Streaming stalls under load; 🔐 Auth cookies stop lying; 🪟 Restore lost preview tabs; 🧪 NaN loss autopsy; 🌲 Worktree setup stops drifting.",
