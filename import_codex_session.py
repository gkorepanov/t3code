...
DEFAULT_PROVIDER_INSTANCE_ID = "codex"
...
def thread_session_set_payload(
    thread_id: str,
    updated_at: str,
    status: str = "ready",
    provider_instance_id: str = DEFAULT_PROVIDER_INSTANCE_ID,
) -> dict:
    return {
        "threadId": thread_id,
        "session": {
...
def upsert_provider_resume_binding(
    ...
    columns = [row["name"] for row in cursor.execute("PRAGMA table_info(provider_session_runtime)").fetchall()]
    has_provider_instance_id = "provider_instance_id" in columns
    provider_instance_insert_column = ", provider_instance_id" if has_provider_instance_id else ""
    provider_instance_insert_value = ", ?" if has_provider_instance_id else ""
    provider_instance_update = ", provider_instance_id = excluded.provider_instance_id" if has_provider_instance_id else ""
    values = [
        thread_id,
        last_seen_at,
    ...
    if has_provider_instance_id:
        values.append(provider_instance_id)
    cursor.execute(
        f"""
        INSERT INTO provider_session_runtime (
          ...
          {provider_instance_insert_column}
        )
        VALUES (?, 'codex', 'codex', 'full-access', 'stopped', ?, ?, ?{provider_instance_insert_value})
...
def latest_turn_id(turns: dict[str, Turn]) -> str | None:
    if not turns:
        return None
    return max(
        turns.values(),
...
def sync_existing_thread(
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        thread_row = cursor.execute(
            """
            SELECT thread_id, project_id, title, model_selection_json, updated_at
        ...
        if thread_row is None:
            raise RuntimeError(f"T3 thread does not exist: {thread_id}")

        existing_message_keys = {
            (row["role"], row["turn_id"], row["text"], row["created_at"])
            for row in cursor.execute(
                """
                SELECT role, turn_id, text, created_at
        ...
        latest_existing_message_at = one_or_none(
            cursor,
            """
        ...
        new_messages = [
            ...
            if (latest_existing_message_at is None or message.created_at > latest_existing_message_at)
            and (message.role, message.turn_id, message.text, message.created_at) not in existing_message_keys
        ]
        new_turn_ids = {message.turn_id for message in new_messages if message.turn_id}
        turns_to_upsert = [
            turn
            for turn_id, turn in session.turns.items()
            if turn_id in new_turn_ids or turn_id not in existing_turn_ids
        ]
        ...
        next_latest_turn_id = latest_turn_id(session.turns)
        model_selection = (
            json.loads(thread_row["model_selection_json"])
            if thread_row["model_selection_json"]
            else {"provider": "codex", "model": session.model}
        )
        command_id = f"codex-sync:existing-thread:{session.codex_id}:{uuid.uuid4()}"

        if dry_run:
            return {
                "status": "dry_run_existing_thread",
                "thread_id": thread_id,
                "provider_resume_thread_id": session.codex_id,
                "messages_to_add": len(new_messages),
                "turns_to_upsert": len(turns_to_upsert),
                "latest_turn_id": next_latest_turn_id,
        ...
        try:
            ...
            cursor.execute(
                ...
                WHERE thread_id = ?
                """,
                (session.updated_at, next_latest_turn_id, latest_user_message_at, thread_id),
            )
            cursor.execute(
                """
                ...
                WHERE thread_id = ?
                """,
                (provider_instance_id, session.codex_id, session.updated_at, thread_id),
            )
            upsert_provider_resume_binding(
                cursor,
                thread_id,
                session,
                model_selection,
    ...
    return {
        "status": "synced_existing_thread",
        "thread_id": thread_id,
        "provider_resume_thread_id": session.codex_id,
        "messages_added": len(new_messages),
        "turns_upserted": len(turns_to_upsert),
        "latest_turn_id": next_latest_turn_id,
        "latest_user_message_at": latest_user_message_at,
        "db_path": str(db_path),
    }
...
def main() -> int:
    ...
    parser.add_argument("--target-thread-id", help="Append/sync into an existing T3 Code thread id.")
    parser.add_argument("--provider-instance-id", default=DEFAULT_PROVIDER_INSTANCE_ID)
